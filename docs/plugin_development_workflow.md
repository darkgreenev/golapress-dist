...
If the plugin is already installed on a site as an installed private `binary` plugin, the Plugins screen can also export it directly through the `Export Package` action. That produces a shareable `.zip` package from the installed plugin directory.
